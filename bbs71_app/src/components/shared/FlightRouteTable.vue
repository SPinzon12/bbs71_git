    ...
    <div class="container">
            ...
            <tbody class="overflow-auto">
                <tr v-for="(route, i) in displayedRoutes" v-bind:key="i">
                    <td class="table-light">
                        <span :style="{
                            'font-weight':
                                $store.state.user.user.type === 'airport' &&
                                    route.origin ===
                                    $store.state.user.user.name.substring(
                    ...
                    <td class="table-light">
                        <span class="text-success">{{ route.count }}</span>
                        <span class="mx-0">/</span>
                        <span class="text-danger">{{ route.cancelled }}</span>
                        <span class="mx-0">/</span>
                        <span class="text-muted">{{ route.diverted }}</span>
                    </td>
                    <td :class="{
                        'text-success':
                            route.routeInfo[0].avgActElapsedTime <=
                            route.routeInfo[0].avgCrsElapsedTime,
                        'text-danger':
                            route.routeInfo[0].avgActElapsedTime >
                            route.routeInfo[0].avgCrsElapsedTime,
                    }" class="table-light">
                        <b>{{ route.routeInfo[0].avgActElapsedTime }}</b>
                    </td>
                    <td class="table-light">
                        {{ route.routeInfo[0].avgCrsElapsedTime }}
                    </td>
        ...
        <div class="row mt-3">
            <div class="col-md-12">
                <p>
                    ...
                    Flights that completed their journey
                </p>
                <p>
                    <span class="text-danger"><i class="fas fa-circle"></i></span> Flights
                    that were cancelled
                </p>
                <p>
                    <span class="text-muted"><i class="fas fa-circle"></i></span> Flights
                    that were diverted
                </p>
            </div>
        </div>
        ...
        <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
            <div class="btn-group mr-2 mx-auto" role="group" aria-label="Button group with nested dropdown">
                ...
                <button v-for="(pageNumber, i) in setRoutes.slice(page - 1, page + 2)" @click="page = pageNumber" :class="{
