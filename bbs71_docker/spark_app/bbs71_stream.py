...
def message_airline(airline_id):
...
def message_airport(airport_id):
...
# Crear un conjunto para almacenar los IDs de mensajes procesados
...
def on_message(client, userdata, msg):
    ...
    if message_id not in processed_messages:
        ...
        if message["type"] == "airline":
            ...
            executor.submit(message_airline, airline_id)
            # message_airline(airline_id)
        else:
            airport_id = message['id']
            print(f"Received message from airport {airport_id}")
            # Ejecutar el procesamiento en paralelo
            executor.submit(message_airport, airport_id)
            # message_airport(airport_id)
