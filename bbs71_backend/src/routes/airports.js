...
router.get("/airlines/:airportID", async (req, res) => {
  try {
    const response = await axios.get(
      `${airportMicroserv}/airports/airlines/${req.params.airportID}`
    );
    res.status(200).json(response.data);
  } catch (error) {
    res.status(500).json({ error: error.message });
...
router.get("/routes/:airportID", async (req, res) => {
  try {
    const response = await axios.get(
      `${airportMicroserv}/airports/routes/${req.params.airportID}`
    );
    res.status(200).json(response.data);
  } catch (error) {
    res.status(500).json({ error: error.message });
